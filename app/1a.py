...
for i, x in enumerate(x0, 1):
    if x < 0 or x >= P:
        print(f"Не выполняется неравенство 0 <= {x} < P для начального условия №{i}")
        exit()

ts = [t0 + i * h for i in range(N + 1)]  # разбиение

xs = []  # список результатов для всех начальных условий x0
for x_start in x0:
    x = [x_start]  # список значений искомой функции x(t)
    for i in range(N):
        f = k * (P - x[i])
        x_next = f * h + x[i]
        x.append(x_next)
    xs.append(x)  # сохраняем результаты для текущего нач. условия
